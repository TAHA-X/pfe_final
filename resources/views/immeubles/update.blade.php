...
@section("body")
    ...
    <form id="add-immeuble-form" enctype="multipart/form-data" action="{{ Route('immeubles.update',$immeuble->id) }}" method="post">
        @csrf
        @method("PUT")
        <div class="mt-2">
            <label class="form-label" for="entrée">entrée :</label>
            <input type="number"  value="{{ $immeuble->entrée }}" class="form-control @error('entrée') is-invalid  @enderror" id="entrée" name="entrée"/>
        </div>
        @error("entrée")
        ...
        <div class="mt-2">
            ...
            <input type="number" value="{{ $immeuble->num }}"  id="num" class="form-control @error('num') is-invalid  @enderror" name="num"/>
        </div>
        @error("num")
           <span class="text-danger">{{ $message }}</span>
        @enderror
            ...
            <div class="mt-2">
                ...
                <select class="form-control" id="projet_id" name="projet_id">
                    @foreach ($projets as $projet)
                        <option @if($projet->id==$le_projet_id) selected @endif value="{{ $projet->id }}">{{ $projet->title }}</option>
                    @endforeach
                </select>
            </div>
