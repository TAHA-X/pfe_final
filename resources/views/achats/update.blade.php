    ...
    <div style="display:flex; justify-content:space-between; flex-wrap:wrap;">
        <div class="bg-dark rounded p-2 text-white" style="width:170px; display:grid; place-items:center; height:100px;">
            @foreach ($projets as $projet)
                @if($projet->id==$achat->projet_id)
                    <span style="font-size:20px; font-weight:bold;">{{ $projet->title }}</span>
                @endif
            @endforeach
        </div>
        <div class="bg-dark rounded p-2 text-white" style="width:170px; display:grid; place-items:center; height:100px;">
            @foreach ($residences as $res)
                  @if($res->id==$achat->residence_id)
                     <span style="font-size:20px; font-weight:bold;">{{ $res->title }}</span>
                  @endif
            @endforeach
        </div>
        <div class="bg-dark rounded p-2 text-white" style="width:170px; display:grid; place-items:center; height:100px;">
